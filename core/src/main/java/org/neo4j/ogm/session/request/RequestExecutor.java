...
public class RequestExecutor {
	...
	public void executeSave(CompileContext context) {
		...
		//If there are nodes to be created and dependent ops, use a long transaction
		if (compiler.hasNewNodes() && compiler.hasUpdates()) {
		...
		//If there are only nodes/no nodes to be created, use autocommit
		if ((compiler.hasNewNodes() && !compiler.hasUpdates()) || (!compiler.hasNewNodes() && compiler.hasUpdates())) {
